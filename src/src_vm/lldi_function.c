...
** Last update Sun Mar 27 19:32:36 2016 Da Fonseca Samuel
*/
...
int	lldi_function(t_vm *vm, t_champ *champ)
{
  ...
  champ->pc = (champ->pc + sum) % MEM_SIZE;
  sum = get_myint(vm, (pc + first_param) % MEM_SIZE, 2) + sec;
  champ->registres[(t - 1) % 16] = get_myint(vm, (pc + sum) % 6144, 4);
  return (0);
}
